...
def llm_response(df: pd.DataFrame) -> list:
    """Function to get response from Mistral 7B model."""
    responses = []
    for prompt in df["prompt"]:
        inputs = tokenizer(prompt, return_tensors="pt").to(device)
        with torch.no_grad():
            outputs = model.generate(**inputs, max_new_tokens=512)  # Adjust as needed
...
giskard_model = giskard.Model(
    ...
    name="Mistral 7B",
    description="A simple text generation model using Mistral 7B.",
...
dataset = Dataset(df=pd.DataFrame(data), target=None, column_types={"prompt": "text"})  
...
with open("Mist1_res.txt", "w", encoding="utf-8") as file:
